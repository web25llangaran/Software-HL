...
public class PiezaEtaMakinakLoturaInportazioa {

  public void inportatu() {

    // Inportatu nahi den fitxategia:
    String piezaMakinakLoturaFitxategia = "../Fitxategiak_DatuZaharrak/pieza_eta_makinak_lotura.csv";
    // Inportazioaren emaitza gordeko da:
    String emaitzaLoga = "../Fitxategiak_DatuZaharrak/log/piezak_eta_makinak.log";

    String[] errenkada;
    int id_pieza, id_makina, kontUdate = 0, kont = 0;
    String lerroa;
    ...
    try {
      ...
      try {
        ...
        BufferedReader br = new BufferedReader(new FileReader(piezaMakinakLoturaFitxategia));
          ...
          while (lerroa != null) {
            ...
            if (errenkada.length == 2) {
              try {
                ...
                String kontsulta = "UPDATE PIEZA SET id_makina = ? WHERE id_pieza = ?";
        ...
        if (errenkadaInportatua) {
          bw.write(LocalDateTime.now() + " INFO: " + kontUdate + " errenkada inportatu dira!\n");
          // System.out.println(kontUdate + " errenkada inportatu dira!");
        } else {
          bw.write(
              LocalDateTime.now()
                  + " ERROR: Akatsak egon dira, ezin izan da "
                  + piezaMakinakLoturaFitxategia
                  + " fitxategia inportatu!\n");
          // System.out.println("Akatsak egon dira, ezin izan da " + piezaMakinakLoturaFitxategia
          // + " fitxategia inportatu!");
        }
        ...
        bw.write(
            LocalDateTime.now()
                ...
                + piezaMakinakLoturaFitxategia
                + " fitxategia aurkitu!\n");
        // System.out.println("Ezin izan da " + piezaMakinakLoturaFitxategia + " fitxategia
        // aurkitu!");
      } catch (IOException e) {
        bw.write(
            LocalDateTime.now()
