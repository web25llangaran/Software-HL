...
public class ErabiltzaileakEtaMakinakInportazioa {

    public void inportatu () {

        // Inportatu nahi den fitxategia:
        String erabiltzaileakEtaMakinakFitxategia = "../Fitxategiak_DatuZaharrak/erabiltzaileak_eta_makinak.csv";
        String emaitzaLoga = "../Fitxategiak_DatuZaharrak/log/erabiltzaileak_eta_makinak.log";

        String[] errenkada;
        int id_erabiltzailea, id_makina, kont = 0, kontInport = 0;
        String lerroa, hasiera_data, amaiera_data;
        ...
        try {
            ...
            try {
                ...
                BufferedReader br = new BufferedReader(new FileReader(erabiltzaileakEtaMakinakFitxategia));
                ...
                if (konexioa != null && !konexioa.isClosed()) {
                    ...
                    while (lerroa != null) {
                        ...
                        if (errenkada.length == 4) {
                            try {
                                id_erabiltzailea = Integer.parseInt(errenkada[0]);
                                id_makina = Integer.parseInt(errenkada[1]);
                                hasiera_data = errenkada[2];
                                amaiera_data = errenkada[3];
                                ...
                                agindua.setInt(2, id_makina);
                                agindua.setDate(3, java.sql.Date.valueOf(hasiera_data));
                                agindua.setDate(4, java.sql.Date.valueOf(amaiera_data));
                ...
                if (errenkadaInportatua) {
                    ...
                    bw.write(LocalDateTime.now() + " ERROR: Akatsak egon dira, ezin izan da " + erabiltzaileakEtaMakinakFitxategia + " fitxategia inportatu!\n");
                    //System.out.println( "Akatsak egon dira, ezin izan da " + erabiltzaileakFitxategia + " fitxategia inportatu!");
                }
            } catch (FileNotFoundException e) {
                bw.write(LocalDateTime.now() + " ERROR: Ezin izan da " + erabiltzaileakEtaMakinakFitxategia + " fitxategia aurkitu!\n");
                //System.out.println("Ezin izan da " + erabiltzaileakFitxategia + " fitxategia aurkitu!");
            } catch (IOException e) {
                bw.write(LocalDateTime.now() + " ERROR: Ezin izan da " + erabiltzaileakEtaMakinakFitxategia + " fitxategia irakurri!\n");
